...
const MOCKUP_DATA_SAMPLES = [
  ...
  { title: 'سناپچات', iconName: 'Ghost', url: 'snapchat.com', color: '#FFFC00', textColor: '#000000' },
...
export default function Home({ user, settings, theme }: Props) {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#f8fafc] flex flex-col font-sans overflow-y-auto overflow-x-hidden relative selection:bg-orange-200 scrollbar-hide pb-[120px]" dir="rtl" style={{ fontFamily: '"Noto Sans Arabic", sans-serif', WebkitOverflowScrolling: 'touch' }}>
      ...
      {/* 🌟 Safe Area ی سەرەوە لێرەدا جێگیر کرا بۆ هێدەر 🌟 */}
      ...
      {/* 🌟 Safe Area ی خوارەوە بۆشایی بۆ دروست کرا تا مۆکئەپ نەچێتە ژێر دوگمەکان 🌟 */}
      <main className="flex-1 flex flex-col lg:flex-row items-center justify-center max-w-7xl mx-auto w-full px-6 gap-10 lg:gap-16 relative z-10 my-auto py-8" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 80px)' }}>
      ...
      {/* 🌟 Safe Area ی خوارەوە لێرەدا جێگیر کرا بۆ دوگمەکان 🌟 */}
      <div className="fixed bottom-0 left-0 w-full px-4 sm:px-6 pt-6 z-40 bg-gradient-to-t from-[#f8fafc] via-[#f8fafc]/90 to-transparent pointer-events-none flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-6 backdrop-blur-[2px]" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 1.5rem)' }}>
