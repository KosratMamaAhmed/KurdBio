...
export default function DraggableLinkList({ links, setLinks, onEdit, onDelete }: any) {
  ...
  const handleDragStart = (e: React.DragEvent, link: any) => {
    setDraggedItem(link);
    // دیاریکردنی جۆری ئیفێکتەکە بۆ ئەوەی جوان بێت لە کاتی ڕاکێشاندا
    e.dataTransfer.effectAllowed = "move";
  };

  const handleDragOver = (e: React.DragEvent) => {
    e.preventDefault(); // پێویستە بۆ ئەوەی ڕێگە بدات Drop بکرێت
  };
  ...
  const handleDrop = (e: React.DragEvent, targetLink: any) => {
    ...
    newLinks.splice(draggedIdx, 1); // سڕینەوە لە شوێنە کۆنەکەی
    newLinks.splice(targetIdx, 0, draggedItem); // دانانی لە شوێنە نوێیەکە

    setLinks(newLinks); // پاشەکەوتکردن لە باکێند
    setDraggedItem(null);
  };
      ...
      {links.map((link: any) => {
        ...
        return (
          ...
          >
            ...
            {/* دوگمەکانی دەستکاریکردن و سڕینەوە */}
