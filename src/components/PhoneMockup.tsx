...
export const THEME_STYLES: Record<string, any> = {
  mockup: { shell: 'border-neutral-800 bg-neutral-900', inner: 'bg-neutral-900', text: 'text-white', sub: 'text-white/60', btn: 'bg-[#1e293b]', iconBg: 'bg-white/5' },
  light: { shell: 'border-neutral-300 bg-white', inner: 'bg-slate-50', text: 'text-neutral-900', sub: 'text-neutral-500', btn: 'bg-white shadow-sm', iconBg: 'bg-slate-100' },
  gold: { shell: 'border-amber-500/40 bg-neutral-950', inner: 'bg-gradient-to-br from-neutral-900 to-black', text: 'text-amber-400', sub: 'text-amber-400/60', btn: 'bg-[#171717]', iconBg: 'bg-amber-500/10' },
  neon: { shell: 'border-cyan-500/30 bg-black', inner: 'bg-gradient-to-br from-slate-900 via-cyan-950 to-black', text: 'text-white', sub: 'text-cyan-200', btn: 'bg-cyan-950/40 shadow-[0_0_15px_rgba(6,182,212,0.3)]', iconBg: 'bg-cyan-500/20' },
  emerald: { shell: 'border-emerald-800 bg-emerald-950', inner: 'bg-gradient-to-br from-emerald-900 to-emerald-950', text: 'text-emerald-50', sub: 'text-emerald-200/70', btn: 'bg-emerald-800/50', iconBg: 'bg-emerald-500/20' },
  vintage: { shell: 'border-orange-800 bg-rose-950', inner: 'bg-gradient-to-br from-rose-900 to-orange-900', text: 'text-rose-50', sub: 'text-rose-200/70', btn: 'bg-rose-950/50', iconBg: 'bg-rose-500/20' },
  crimson: { shell: 'border-red-900 bg-red-950', inner: 'bg-gradient-to-br from-red-900 to-red-950', text: 'text-red-50', sub: 'text-red-200/70', btn: 'bg-red-900/80', iconBg: 'bg-red-800' },
  navy: { shell: 'border-blue-800 bg-blue-950', inner: 'bg-gradient-to-br from-blue-900 to-slate-900', text: 'text-blue-50', sub: 'text-blue-200/70', btn: 'bg-blue-800/40', iconBg: 'bg-blue-500/20' },
  royal: { shell: 'border-amber-500/40 bg-purple-950', inner: 'bg-gradient-to-br from-purple-950 to-indigo-950', text: 'text-amber-400', sub: 'text-amber-400/60', btn: 'bg-purple-900/50', iconBg: 'bg-amber-500/10' },
  minimal: { shell: 'border-gray-300 bg-gray-100', inner: 'bg-gray-200', text: 'text-gray-900', sub: 'text-gray-600', btn: 'bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]', iconBg: 'bg-gray-100' },
  cyberpunk: { shell: 'border-yellow-400 bg-neutral-900', inner: 'bg-[#fce300]', text: 'text-black', sub: 'text-neutral-800', btn: 'bg-black shadow-[4px_4px_0px_0px_rgba(6,182,212,1)]', iconBg: 'bg-neutral-800' },
  glassmorphism: { shell: 'border-white/20 bg-slate-900', inner: 'bg-[radial-gradient(circle_at_top_left,_var(--tw-gradient-stops))] from-indigo-500 via-purple-500 to-pink-500', text: 'text-white', sub: 'text-white/80', btn: 'bg-white/10 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.1)]', iconBg: 'bg-white/20' },
  dracula: { shell: 'border-[#ff79c6] bg-[#282a36]', inner: 'bg-[#282a36]', text: 'text-[#f8f8f2]', sub: 'text-[#6272a4]', btn: 'bg-[#44475a]', iconBg: 'bg-[#6272a4]/20' },
  aurora: { shell: 'border-green-400/30 bg-[#0B101E]', inner: 'bg-gradient-to-b from-[#0B101E] to-[#1A2F2B]', text: 'text-green-300', sub: 'text-green-300/60', btn: 'bg-[#121E1C] shadow-[0_0_15px_rgba(34,197,94,0.1)]', iconBg: 'bg-green-500/10' },
  sunset: { shell: 'border-orange-400/30 bg-[#2D1B2E]', inner: 'bg-gradient-to-br from-[#FF512F] to-[#DD2476]', text: 'text-white', sub: 'text-white/80', btn: 'bg-white/10 backdrop-blur-sm', iconBg: 'bg-white/20' },
  ocean: { shell: 'border-cyan-500/30 bg-[#0A192F]', inner: 'bg-gradient-to-b from-[#0A192F] to-[#020c1b]', text: 'text-[#64ffda]', sub: 'text-[#8892b0]', btn: 'bg-[#112240]', iconBg: 'bg-[#64ffda]/10' },
  forest: { shell: 'border-[#A3B18A] bg-[#344E41]', inner: 'bg-[#3A5A40]', text: 'text-[#DAD7CD]', sub: 'text-[#A3B18A]', btn: 'bg-[#588157]', iconBg: 'bg-[#344E41]' },
  candy: { shell: 'border-pink-300 bg-pink-50', inner: 'bg-gradient-to-br from-pink-200 to-sky-200', text: 'text-slate-800', sub: 'text-slate-600', btn: 'bg-white/60 backdrop-blur-sm shadow-xl', iconBg: 'bg-white' },
  hacker: { shell: 'border-[#00ff00] bg-black', inner: 'bg-black', text: 'text-[#00ff00]', sub: 'text-[#00ff00]/60', btn: 'bg-black', iconBg: 'bg-[#00ff00]/10' },
  luxury: { shell: 'border-[#B76E79] bg-[#1a1a1a]', inner: 'bg-gradient-to-b from-[#2c2c2c] to-[#1a1a1a]', text: 'text-[#B76E79]', sub: 'text-[#B76E79]/70', btn: 'bg-[#2a2a2a] shadow-[0_4px_15px_rgba(183,110,121,0.2)]', iconBg: 'bg-[#B76E79]/10' }
};
...
export default function PhoneMockup({ mockup, mockupLinks, sponsoredLinks, isPublic = false, handleLinkClick }: any) {
  ...
  // 🌟 لێرەدا قەبارەی مۆبایلەکە ڕێکخراوە بۆ ئەوەی ڕێژەی 19:9 بێت و هەرگیز پان نەبێتەوە 🌟
  // aspect-[9/19] وادەکات شێوەی مۆبایلەکە بەبێ تێکچوون بپارێزرێت تەنانەت لەسەر شاشەی گەورەش
  ...
  return (
    <div className={containerClass}>
      <div className={`w-full h-full ${innerRounded} relative overflow-hidden flex flex-col ${t.inner}`}>
        ...
        <div className={`flex-1 flex flex-col items-center pt-14 px-4 pb-6 z-20 w-full overflow-y-auto scrollbar-hide ${isPublic ? 'pb-32' : ''}`}>
          ...
          <div className="w-full space-y-4 px-1 sm:px-2 flex-none">
            {displayLinks.map((link: any, idx: number) => {
               ...
               const btnContent = (
                  // 🌟 لێرەدا ئیفێکتی ئاگرەکە چاککرا بۆ ئەوەی تەنها بە لێوارەکاندا بسوڕێتەوە 🌟
                  <div className={`relative w-full rounded-2xl p-[1.5px] overflow-hidden group transition-transform hover:scale-[1.03] active:scale-95 shadow-[0_5px_20px_rgba(0,0,0,0.2)]`}>
                    
                    {/* باکگراوندی ژێرەوەی لێوارەکە بۆ ئەوەی ئاگرەکە لەسەری دەربکەوێت */}
                    <div className="absolute inset-0 bg-white/10 z-0"></div>

                    {/* ئیفێکتی ئاگرەکە (دەسوڕێتەوە) */}
                    <div className="absolute top-1/2 left-1/2 w-[300%] aspect-square -translate-x-1/2 -translate-y-1/2 animate-[spin_2.5s_linear_infinite] z-0">
                      <div className="absolute inset-0 blur-[6px]" style={{ background: `conic-gradient(from 90deg, transparent 0 200deg, ${customColor}50 280deg, ${customColor} 360deg)` }}></div>
                      <div className="absolute top-1/2 right-[10%] w-8 h-8 -translate-y-1/2 rounded-full blur-[4px] opacity-100" style={{ backgroundColor: customColor, boxShadow: `0 0 20px 8px ${customColor}` }}></div>
                      <div className="absolute top-1/2 right-[12%] w-2 h-2 -translate-y-1/2 bg-white rounded-full blur-[1px]"></div>
                    </div>

                    {/* دوگمە ئەسڵییەکە کە دەکەوێتە سەرەوە (زۆربەی ئاگرەکە دادەپۆشێت تەنها لێوارەکە دەهێڵێتەوە) */}
                    <div className={`relative z-10 w-full rounded-[15px] flex items-center justify-between p-2.5 ${t.btn}`} style={['gold', 'light', 'cyberpunk'].includes(designId) ? { backgroundColor: designId === 'gold' ? '#171717' : designId === 'light' ? '#ffffff' : '#000000' } : {}}>
                      <div className="flex items-center gap-3 w-full pr-1">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 shadow-inner overflow-hidden ${t.iconBg}`}>
                        ...
                        <span className={`font-black text-[15px] truncate ${btnTextClass}`} style={btnTextStyle}>
              ...
              {displaySponsored.map((item: any, idx: number) => {
                 ...
                 const spBtnContent = (
                    <div className="relative w-full group transition-transform hover:scale-[1.03] active:scale-95 shadow-[0_10px_30px_-10px_rgba(239,68,68,0.5)]">
                      <div className="absolute -top-1.5 -left-2.5 bg-gradient-to-r from-red-600 to-orange-500 text-white text-[10px] font-black px-2.5 py-0.5 rounded-full shadow-[0_0_15px_rgba(239,68,68,0.8)] border border-yellow-300 z-30 rotate-[-12deg]">VIP</div>
                      <div className="relative w-full rounded-2xl p-[2px] overflow-hidden">
                        
                        {/* 🔥 ئیفێکتی ئاگری VIP بە لێوارەکاندا 🔥 */}
                        <div className="absolute inset-0 bg-white/10 z-0"></div>
                        <div className="absolute top-1/2 left-1/2 w-[300%] aspect-square -translate-x-1/2 -translate-y-1/2 animate-[spin_1.5s_linear_infinite] z-0">
                           <div className="absolute inset-0 opacity-100 blur-[8px]" style={{ background: `conic-gradient(from 90deg, transparent 0 160deg, #ef4444 240deg, #f97316 300deg, #fbbf24 360deg)` }}></div>
                        ...
                        {/* دوگمەی VIP */}
                        <div className="relative z-10 w-full bg-[#0a0a0a] rounded-[14px] p-3 sm:p-4 flex items-center justify-between">
                          <div className="flex items-center gap-3 w-full pr-1 mt-1">
                            <div className="w-14 h-14 rounded-xl bg-white/5 p-1.5 flex items-center justify-center shrink-0 shadow-[inset_0_0_20px_rgba(255,255,255,0.05)] border border-white/10 relative overflow-hidden">
        ...
        {/* 🌟 دوگمەی خوارەوە بە جوانی جێگیر کراوە 🌟 */}
        {isPublic && (
