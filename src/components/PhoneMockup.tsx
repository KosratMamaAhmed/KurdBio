...
const THEME_STYLES: Record<string, any> = {
  ...
  light: { shell: 'border-neutral-300 bg-white', inner: 'bg-slate-50', text: 'text-neutral-900', sub: 'text-neutral-500', btn: 'bg-white border-neutral-200', iconBg: 'bg-slate-100' },
  ...
  neon: { shell: 'border-cyan-500/30 bg-black', inner: 'bg-gradient-to-br from-slate-900 via-cyan-950 to-black', text: 'text-white', sub: 'text-cyan-200', btn: 'bg-cyan-950/40 border-cyan-500/50 shadow-[0_0_10px_rgba(6,182,212,0.2)]', iconBg: 'bg-cyan-500/20' },
...
export default function PhoneMockup({ mockup, mockupLinks, sponsoredLinks, isPublic = false, handleLinkClick }: any) {
  ...
  const innerRounded = isPublic ? "rounded-none sm:rounded-[2.8rem]" : "rounded-[2.5rem] sm:rounded-[3rem]";
  ...
  return (
    <div className={containerClass}>
      <div className={`w-full h-full ${innerRounded} relative overflow-hidden flex flex-col ${t.inner}`}>
        ...
        <div className={`absolute top-3 left-1/2 -translate-x-1/2 w-24 h-6 sm:h-7 bg-black rounded-full z-30 items-center justify-center gap-2 shadow-sm border border-white/5 ${isPublic ? 'hidden sm:flex' : 'flex'}`}>
          <div className="w-1.5 h-1.5 rounded-full bg-white/20"></div>
          <div className="w-1.5 h-1.5 rounded-full bg-white/10"></div>
        </div>

        <div className={`flex-1 flex flex-col items-center pt-12 sm:pt-16 px-4 pb-6 z-20 w-full ${isPublic ? 'overflow-y-auto scrollbar-hide pb-32' : 'overflow-hidden pb-6'}`}>
              ...
              {displaySponsored.map((item: any, idx: number) => {
                 ...
                 const spBtnContent = (
                    <div className="relative w-full group transition-transform hover:scale-[1.03] active:scale-95 shadow-[0_10px_30px_-10px_rgba(239,68,68,0.5)]">
                      <div className="absolute -top-1 -left-2 bg-gradient-to-r from-red-600 to-orange-500 text-white text-[9px] sm:text-[10px] font-black px-2 py-0.5 rounded-full shadow-[0_0_10px_rgba(239,68,68,0.8)] border border-yellow-300 z-30 rotate-[-12deg]">
                        2026
                      </div>
          ...
          {!isPublic && (
             <div className="mt-auto pt-6 pb-2 w-full flex justify-center shrink-0 relative z-20">
                <span className="text-amber-400 font-black text-[11px] sm:text-[13px] drop-shadow-[0_2px_8px_rgba(245,158,11,0.5)] text-center tracking-wide w-full px-2">
                   بە ئاسانی دیزاینێکی ئاوا بۆخۆت دروست بکە
                </span>
             </div>
          )}
        </div>
          ...
          <div className="absolute bottom-0 left-0 w-full pt-12 pb-6 sm:pb-8 px-4 bg-gradient-to-t from-black via-black/80 to-transparent z-40 flex justify-center pointer-events-none">
            <a href="https://biokurd.com" className="relative p-[2.5px] rounded-full overflow-hidden group hover:scale-105 active:scale-95 transition-all duration-300 w-full max-w-[300px] shadow-[0_0_40px_-10px_rgba(251,191,36,0.5)] pointer-events-auto">
              ...
              <div className="relative z-10 flex items-center justify-center gap-3 px-5 py-3 sm:py-3.5 bg-gradient-to-r from-neutral-950 via-neutral-900 to-neutral-950 backdrop-blur-xl rounded-full w-full h-full border border-white/10 overflow-hidden shadow-[inset_0_0_20px_rgba(251,191,36,0.05)]">
                ...
                <span className="text-amber-400 font-bold text-[16px] sm:text-[18px] whitespace-nowrap drop-shadow-[0_2px_10px_rgba(245,158,11,0.6)] relative z-10 tracking-wide mt-0.5" style={{ fontFamily: '"Amiri", serif' }}>
                  لینکێکی ئاوا بۆخۆت دروست بکە
                </span>
              </div>
            </a>
          </div>
      ...
      <div className={`absolute top-20 -left-1 w-1.5 h-8 bg-neutral-800 rounded-l-md ${isPublic ? 'hidden sm:block' : 'block'}`}></div>
      <div className={`absolute top-36 -left-1 w-1.5 h-12 bg-neutral-800 rounded-l-md ${isPublic ? 'hidden sm:block' : 'block'}`}></div>
      <div className={`absolute top-52 -left-1 w-1.5 h-12 bg-neutral-800 rounded-l-md ${isPublic ? 'hidden sm:block' : 'block'}`}></div>
      <div className={`absolute top-36 -right-1 w-1.5 h-16 bg-neutral-800 rounded-r-md ${isPublic ? 'hidden sm:block' : 'block'}`}></div>
    </div>
  );
}