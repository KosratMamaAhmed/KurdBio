...
const CARD_DESIGNS = [
  { id: 0, name: 'ئاڵتوونی شاهانە (Luxury Gold)', textName: '#111111', textBio: '#4b5563', qrFg: '#111111', customTxtCol: '#ffffff', accent: '#d4af37' },
  { id: 1, name: 'یاقووتی مۆدێرن (Modern Red)', textName: '#0f172a', textBio: '#475569', qrFg: '#991b1b', customTxtCol: '#ffffff', accent: '#dc2626' },
  { id: 2, name: 'تاریکی ئەندازەیی (Dark Geo)', textName: '#f8fafc', textBio: '#94a3b8', qrFg: '#0f172a', customTxtCol: '#ffffff', accent: '#3b82f6' },
  { id: 3, name: 'خۆرئاوابوون (Sunset Curve)', textName: '#f8fafc', textBio: '#a1a1aa', qrFg: '#9a3412', customTxtCol: '#ffffff', accent: '#f97316' },
  { id: 4, name: 'شەپۆلی ئاسمانی (Azure Wave)', textName: '#0f172a', textBio: '#64748b', qrFg: '#0284c7', customTxtCol: '#ffffff', accent: '#0ea5e9' }
];
...
export default function Card({ profile, onClose }: CardProps) {
  ...
  const [fontSize, setFontSize] = useState(18); // Size: 18px
  const [posX, setPosX] = useState(191); // X: 191
  const [posY, setPosY] = useState(541); // Y: 541
  ...
  const generateCardPreview = async () => {
    ...
    try {
      ...
      if (activeDesign === 0) {
          ...
          ctx.beginPath(); ctx.moveTo(400, 0); ctx.lineTo(1050, 0); ctx.lineTo(1050, 600); ctx.lineTo(250, 600); ctx.fill();
          ctx.strokeStyle = design.accent; ctx.lineWidth = 15;
          ctx.beginPath(); ctx.moveTo(400, -10); ctx.lineTo(250, 610); ctx.stroke();
      } 
      else if (activeDesign === 1) {
          ctx.fillStyle = '#f8fafc'; ctx.fillRect(0, 0, 1050, 600);
          ctx.fillStyle = design.accent;
          ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(380, 0); ctx.lineTo(550, 600); ctx.lineTo(0, 600); ctx.fill();
          ctx.fillStyle = '#991b1b'; 
          ctx.beginPath(); ctx.moveTo(380, 0); ctx.lineTo(410, 0); ctx.lineTo(580, 600); ctx.lineTo(550, 600); ctx.fill();
      }
      else if (activeDesign === 2) {
          ctx.fillStyle = '#1e293b'; ctx.fillRect(0, 0, 1050, 600);
          ctx.fillStyle = '#020617';
          ctx.beginPath(); ctx.moveTo(450, 0); ctx.lineTo(1050, 0); ctx.lineTo(1050, 600); ctx.lineTo(280, 600); ctx.fill();
          ctx.fillStyle = design.accent;
          ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(250, 0); ctx.lineTo(0, 250); ctx.fill();
      }
      else if (activeDesign === 3) {
          ctx.fillStyle = '#09090b'; ctx.fillRect(0, 0, 1050, 600);
          const grad = ctx.createLinearGradient(0, 0, 450, 600);
          grad.addColorStop(0, '#f97316'); grad.addColorStop(1, '#eab308');
          ctx.fillStyle = grad;
          ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(450, 0); ctx.bezierCurveTo(400, 300, 550, 450, 300, 600); ctx.lineTo(0, 600); ctx.fill();
      }
      else if (activeDesign === 4) {
          ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, 1050, 600);
          const bgRight = ctx.createLinearGradient(0, 0, 1050, 600);
          bgRight.addColorStop(0, '#f1f5f9'); bgRight.addColorStop(1, '#e2e8f0');
          ctx.fillStyle = bgRight; ctx.fillRect(0, 0, 1050, 600);
          
          ctx.fillStyle = design.accent;
          ctx.beginPath(); ctx.moveTo(0, 450); ctx.bezierCurveTo(400, 350, 600, 550, 1050, 400); ctx.lineTo(1050, 600); ctx.lineTo(0, 600); ctx.fill();
          ctx.fillStyle = '#0284c7';
          ctx.beginPath(); ctx.moveTo(0, 520); ctx.bezierCurveTo(500, 450, 700, 600, 1050, 480); ctx.lineTo(1050, 600); ctx.lineTo(0, 600); ctx.fill();
      }

      ctx.save();
      ctx.translate(525, 300); ctx.rotate(-Math.PI / 8);
      ctx.fillStyle = (activeDesign === 0 || activeDesign === 1 || activeDesign === 4) ? 'rgba(0, 0, 0, 0.02)' : 'rgba(255, 255, 255, 0.02)'; 
      ...
      const xQ = 60; const yQ = 140; const wQ = 260; const hQ = 260;

      ctx.fillStyle = '#FFFFFF';
      roundRect(ctx, xQ - 12, yQ - 12, wQ + 24, hQ + 24, 20); ctx.fill(); 
      ctx.strokeStyle = design.accent; ctx.lineWidth = 2; ctx.stroke();
      ...
      try {
        ...
        ctx.save(); roundRect(ctx, xQ, yQ, wQ, hQ, 10); ctx.clip(); ctx.drawImage(qrImg, xQ, yQ, wQ, hQ); ctx.restore();
      } catch (e) { console.error("Failed to load QR code"); }


      // 🛑 لابردنی هەموو سێبەرێک بۆ سافکردنی نووسینەکان 🛑
      ctx.shadowBlur = 0; ctx.shadowColor = 'transparent'; ctx.shadowOffsetX = 0; ctx.shadowOffsetY = 0;

      const textCenterX = 730; const avatarY = 160; const avatarRadius = 75;

      ctx.beginPath(); ctx.arc(textCenterX, avatarY, avatarRadius + 6, 0, Math.PI * 2); ctx.fillStyle = design.accent; ctx.fill();
      ctx.beginPath(); ctx.arc(textCenterX, avatarY, avatarRadius + 2, 0, Math.PI * 2); ctx.fillStyle = (activeDesign === 0 || activeDesign === 1 || activeDesign === 4) ? '#ffffff' : '#020617'; ctx.fill();
      ...
      if (profile.avatarUrl) {
        ...
        } catch (e) { drawAvatarFallback(ctx, textCenterX, avatarY, avatarRadius, profile.displayName, fontPrimary, activeDesign === 0 || activeDesign === 1 || activeDesign === 4); }
      } else { drawAvatarFallback(ctx, textCenterX, avatarY, avatarRadius, profile.displayName, fontPrimary, activeDesign === 0 || activeDesign === 1 || activeDesign === 4); }

      const nameY = 280; ctx.textBaseline = 'top'; ctx.textAlign = 'center';
      ctx.fillStyle = debouncedSettings.nameCol;
      ctx.font = `900 42px ${fontPrimary}`; 
      ctx.fillText(profile.displayName || 'کۆسرەت', textCenterX, nameY, 400);

      ctx.fillStyle = debouncedSettings.bioCol; 
      ctx.font = `500 24px ${fontPrimary}`;
      const bioText = profile.bio || 'باشترین بەستەرەکانم لێرە ببینە';
      wrapText(ctx, bioText, textCenterX, 345, 34, 380);

      const customRenderText = debouncedSettings.text.trim();
      if (customRenderText !== '') {
          ctx.fillStyle = debouncedSettings.customCol;
          // قەبارە و شوێن ڕێک بەو ژمارانەی دیاریکراون
  ...
  const drawAvatarFallback = (ctx: CanvasRenderingContext2D, x: number, y: number, r: number, name: string, font: string, isLight: boolean) => {
    ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fillStyle = isLight ? '#f1f5f9' : '#1e293b'; ctx.fill();
    ctx.fillStyle = CARD_DESIGNS[activeDesign].accent; ctx.font = `bold 65px ${font}`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
